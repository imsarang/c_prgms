...
bool dfs(vector<vector<char>>Graph, char c[], int i, int j, int N, int M,
         bool **visited) {
  ...
  if (i - 1 >= 0 && j - 1 >= 0 && Graph[i - 1][j - 1] == c[0] &&
      !visited[i - 1][j - 1])
  ...
  if (i - 1 >= 0 && j + 1 < M && Graph[i - 1][j + 1] == c[0] &&
      !visited[i - 1][j + 1])
    ans = ans || dfs(Graph, c + 1, i - 1, j + 1, N, M, visited);
  ...
  if (i + 1 < N && j + 1 < M && Graph[i + 1][j + 1] == c[0] &&
      !visited[i + 1][j + 1])
    ans = ans || dfs(Graph, c + 1, i + 1, j + 1, N, M, visited);
  ...
  if (i + 1 < N && j - 1 >= 0 && Graph[i + 1][j - 1] == c[0] &&
      !visited[i + 1][j - 1])
    ans = ans || dfs(Graph, c + 1, i + 1, j - 1, N, M, visited);
